	...
	if ( input.open( QIODevice::ReadOnly ) ) {
		...
		for ( int i = 0 ; i < l.count(); i++ ) {
			...
			if ( tagName == "meta" )
				...
				kDebug() << "Unknown tag within <recipeml>: " << tagName << endl;
		}
	}
	else
...
void RecipeMLImporter::readRecipemlRecipe( const QDomElement& recipe_element )
{
	...
	for ( int i = 0; i < l.count(); i++ ) {
		...
		if ( tagName == "head" )
			...
			kDebug() << "Unknown tag within <recipe>: " << el.tagName() << endl;
	}
		...
		if ( tagName == "title" )
			...
			for ( int j = 0; j < yieldChildren.count(); j++ ) {
				...
				if ( tagName == "range" )
					...
					kDebug() << "Unknown tag within <yield>: " << y.tagName() << endl;
			}
		}
		else
			kDebug() << "Unknown tag within <head>: " << el.tagName() << endl;
	}
}
		...
		else if ( tagName == "ing-div" )  //NOTE: this can have the "type" attribute
		{
			...
			for ( int j = 0; j < ingDiv.count(); j++ )
			{
				...
				if ( cEl.tagName() == "title" )
					...
					kDebug() << "Unknown tag within <ing-div>: " << cEl.tagName() << endl;
			}
		}
	else if ( tagName == "note" ) {} //TODO: what do we do with this?
		else
			kDebug() << "Unknown tag within <ingredients>: " << el.tagName() << endl;
	}
}
		...
		if ( tagName == "amt" ) {
			...
			for ( int k = 0; k < amtChilds.count(); k++ ) {
				...
				if ( amtChild.tagName() == "qty" )
					...
					kDebug() << "Unknown tag within <amt>: " << amtChild.tagName() << endl;
			}
		}
			...
			kDebug() << "Unknown tag within <ing>: " << ingChild.tagName() << endl;
	}
...
void RecipeMLImporter::readRecipemlDirections( const QDomElement& dirs )
{
	...
	for ( int i = 0 ; i < l.count(); i++ ) {
		...
		if ( el.tagName() == "step" )
			...
			kDebug() << "Unknown tag within <directions>: " << el.tagName() << endl;
	}
...
void RecipeMLImporter::readRecipemlMenu( const QDomElement& menu_el )
{
	...
	for ( int i = 0 ; i < l.count(); i++ ) {
		...
		if ( tagName == "head" )
			...
			kDebug() << "Unknown tag within <menu>: " << tagName << endl;
	}
}
...
void RecipeMLImporter::readRecipemlSrcItems( const QDomElement& sources )
{
	...
	for ( int i = 0 ; i < l.count(); i++ ) {
		...
		if ( tagName == "srcitem" )
			...
			kDebug() << "Unknown tag within <source>: " << tagName << endl;
	}
}
		...
		if ( tagName == "time" ) {
			...
			for ( int i = 0 ; i < time_l.count(); i++ ) {
				...
				if ( time_tagName == "qty" )
					...
					kDebug() << "Unknown tag within <time>: " << time_tagName << endl;
			}
			...
			if ( timeunit == "minutes" )
				...
				kDebug() << "Unknown timeunit: " << timeunit << endl;

			recipe.prepTime = QTime( hours + minutes / 60, minutes % 60 );
		}
		else
			kDebug() << "Unknown tag within <preptime>: " << tagName << endl;
	}
}
...
void RecipeMLImporter::readRecipemlQty( const QDomElement &qty, Ingredient &ing )
{
	...
	for ( int i = 0; i < qtyChilds.count(); i++ ) {
		...
		if ( tagName == "range" )
			...
			kDebug() << "Unknown tag within <qty>: " << tagName << endl;
	}
}
...
void RecipeMLImporter::readRecipemlRange( const QDomElement& range, double &amount, double &amount_offset )
{
	...
	for ( int j = 0; j < rangeChilds.count(); j++ ) {
		...
		if ( subTagName == "q1" )
			...
			kDebug() << "Unknown tag within <range>: " << subTagName << endl;
	}
