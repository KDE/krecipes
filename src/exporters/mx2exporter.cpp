...
// Both MasterCook and krecipes have ingredient-groups/subtitles in the recipe list. They are analogous. The difference is that 
// while krecipes has ingredient-groups as parent elements to ingredients, MasterCook does not distinguish. 
// In MasterCook both are ingredients except that an ingredient that is really a group/subtitle header has an attribute "code" 
// with the value "S".
// After that, the big difference is that MasterCook treats name, units, and quantity as atributes of an ingredient while krecipes
// treats them as separate elements.
//
// So we will crawl through the groups and ingredients...
// We will handle the cases of whether there are ingredient-groups or not as separate cases.
